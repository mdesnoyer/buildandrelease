...
template node[:bc_controller][:ingester_config] do
  ...
  variables({
              ...
              :carbon_host => node[:neon][:carbon_host],
              :carbon_port => node[:neon][:carbon_port],
              :service_name => node[:bc_controller][:bc_service_name], 
              :flume_log_port => node[:neon_logs][:json_http_source_port],
            })
end
...
template node[:bc_controller][:cnn_ingester_config] do
  ...
  variables({
              ...
              :carbon_host => node[:neon][:carbon_host],
              :carbon_port => node[:neon][:carbon_port],
              :service_name => node[:bc_controller][:cnn_service_name], 
              :flume_log_port => node[:neon_logs][:json_http_source_port],
            })
end

template node[:bc_controller][:fox_ingester_config] do
  source "ingester.conf.erb"
  owner "bc_controller"
  group "bc_controller"
  mode "0644"
  variables({
              ...
              :cmsapi_host => cmsapi_host, 
              :cmsapi_port => node[:cmsapi][:port], 
              :log_file => node[:bc_controller][:fox_ingester_log_file],
              :carbon_host => node[:neon][:carbon_host],
              :carbon_port => node[:neon][:carbon_port],
              :service_name => node[:bc_controller][:fox_service_name], 
              :flume_log_port => node[:neon_logs][:json_http_source_port],
            })
end

template node[:bc_controller][:microsoft_ingester_config] do
  source "ingester.conf.erb"
  owner "bc_controller"
  group "bc_controller"
  mode "0644"
  variables({
              ...
              :cmsapi_host => cmsapi_host, 
              :cmsapi_port => node[:cmsapi][:port], 
              :log_file => node[:bc_controller][:microsoft_ingester_log_file],
              :carbon_host => node[:neon][:carbon_host],
              :carbon_port => node[:neon][:carbon_port],
              :service_name => node[:bc_controller][:microsoft_service_name], 
              :flume_log_port => node[:neon_logs][:json_http_source_port],
            })
end
